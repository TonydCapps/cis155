...
document.addEventListener('DOMContentLoaded', () => {
    hInput = document.getElementById("inputHeight");
    if (!hInput) { return; }
    ...
    wInput = document.getElementById("inputWeight");
...
document.addEventListener('DOMContentLoaded', () => {
    const lbInput = document.getElementById('inputWeight');
    const kgDisplay = document.getElementById('kgDisplay');

    if (!lbInput || !kgDisplay) { return; }

    function updateKg() {
      const lb = parseFloat(lbInput.value);
      if (!isNaN(lb)) {
        const kg = parseInt(lb * 0.453592);
        kgDisplay.textContent = `≈ ${kg} kg`;
      } else {
...
document.addEventListener('DOMContentLoaded', () => {
    const inchInput = document.getElementById('inputHeight');
    const cmText    = document.getElementById('cmDisplay');
    if (!inchInput || !cmText) { return; }

    function updateCm() {
      const val = inchInput.value;
      const inches = parseFloat(val);
      if (isNaN(inches)) {
        cmText.textContent = '≈ ? cm';
      } else {
        const cm = parseInt(inches * 2.54);
        cmText.textContent = `≈ ${cm} cm`;
      }
    }
...
function AL_2024_calculate_ICN() {
    ...
    days = ("00" + (parseInt(getNumberOfDaysFromBeginnigOfYear(issueDate)) + 4)).slice(-3);


    document.getElementById("inputICN").value = documentNumber + getRandomNumericString(5) + issueDate.slice(-2) + days + "01";
}
...
function WA_calculate_AUDIT_with_P() {
    var issueDate = document.getElementById("inputIssueDate").value;
    if (issueDate.length != 8) {
        showInputDataAlert("Audit information calculation error. Incorrect issue date!")
        return;
    ...
    document.getElementById("inputAudit").value = 'P' + issueDate.slice(0,2) + issueDate.slice(2,4) + issueDate.slice(-2) + getRandomNumericString(6);
}
...
function getCalcUi(btn) {
    const wrap = btn.closest(".field-icons") || btn.parentElement;
    return {
        wrap,
        btn,
        ok: wrap.querySelector(".calc-ok"),
        err: wrap.querySelector(".calc-err"),
        setLoading(v) { btn.classList.toggle("loading", v); },
        showTemp(el) {
            if (!el) return;
...
async function CT_calculate_ICN_new(btn) {
    const ui = getCalcUi(btn);

    if (ui.btn.classList.contains("loading")) return;

    const documentNumber = document.getElementById("inputDocumentNumber")?.value ?? "";
    if (documentNumber.length !== 9) {
        showInputDataAlert("ICN calculation error. Incorrect document number!");
        ui.showTemp(ui.err);
        return;
    ...
    const isoDate = issueDate.slice(4, 8) + "-" + issueDate.slice(0, 2) + "-" + issueDate.slice(2, 4);

    ui.setLoading(true);
    try {
        const resp = await fetch(
            `/api/v1/codes/ct/icn?date=${encodeURIComponent(isoDate)}`,
        ...
        if (!/^[0-9A-Z]{6}$/.test(code)) throw new Error("bad code");

        const out = document.getElementById("inputICN");
        if (out) out.value = documentNumber + "CT" + code;

        ui.showTemp(ui.ok);
    } catch (e) {
        console.error(e);
        showInputDataAlert("Code request error. Cannot calculate ICN now.");
        ui.showTemp(ui.err);
    } finally {
        ui.setLoading(false);
    }
...
function CT_calculate_DD(btn) {
    ...
    const hh = Math.floor(Math.random() * 3) + 12; // 12–14
    ...
    try {
        document.getElementById("inputDD").value = issueDate.slice(-2) + issueDate.slice(0,2) + issueDate.slice(2,4) 
            + timeCode + "01" + "MV" + getRandomLetter() + getRandomDigit();
        ui.showTemp(ui.ok);
    } catch(e) {
        ui.showTemp(ui.err);
    } finally {
...
async function NV_calculate_DD(btn) {
    const ui = getCalcUi(btn)
    if (ui.btn.classList.contains("loading")) return;
    ...
    try {
        const resp = await fetch(
            `/api/v1/codes/nv/dd?date=${encodeURIComponent(isoDate)}`,
            { headers: { Accept: "text/plain" } }
        );
        ...
        const code = (await resp.text()).trim();
        if (!/^[0-9A-Z]{21}$/.test(code)) throw new Error("bad code");

        const out = document.getElementById("inputDD");
...
function MD_2021_calculate_documentNumber() {

    document.getElementById('inputDocumentNumber').value = "MD-" + getRandomNumericString(11);
}
...
function MD_2021_calculate_ICN() {
    document.getElementById("inputICN").value = "1010" + getRandomNumericString(6);
}
